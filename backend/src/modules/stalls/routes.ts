...
export default async function stallsRoutes(app: FastifyInstance) {
  app.get("/stalls", async () => {
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      if (!parse.success) {
        request.log.warn({ issues: parse.error.issues }, "stalls.update validation failed");
      ...
      request.log.info("stalls.update success");
      return parse.data;
    }
  );
